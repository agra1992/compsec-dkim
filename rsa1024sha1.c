...
doSign(RSA *prikey, RSA *pubkey, unsigned char *data, int dataLen)
{
	...
	/* Sign */
	//Sign the message digest hash wtih RSA private key of length 1024 bits
	...
	/* Verify */
	ret = RSA_verify(NID_sha1, hash, SHA1_DIGEST_LENGTH, sign,
					 signLen, pubkey);
	printf("RSA_Verify: %s\n", (ret == 1) ? "true" : "false");
...
main()
{
	...
	*/
	...
	priKeyFile = fopen("rsaprivatekey1024.pem", "w");
	if (priKeyFile == NULL)	{
    ...
    */
	printf("\n< RSA Sign with 1024 PriKey and SHA1 >\n");
